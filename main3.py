...
async def search_players(
    ...
    """Search players from fut_players table"""
    query = q.strip()
    ...
    try:
        # Search query - adjust column names if they differ from your actual table
        sql = """
        ...
        for row in rows:
            players.append({
                "id": row["card_id"],  # Use card_id as the ID
                "name": row["name"],
                "rating": row["rating"],
                "version": row["version"] or "Base",
                "card_id": str(row["card_id"]),  # Ensure it's a string
                "image_url": row["image_url"],
                "player_slug": row["player_slug"],
                "player_url": row["player_url"],
                "club": "Unknown",  # You might want to add these fields to your DB
                "nation": "Unknown",
                "position": "Unknown"
            })
