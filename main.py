...
# -------------------
# Config (no hard fail at import time)
# -------------------
...
async def check_server_membership(user_id: str):
    # If not configured, allow all (change to False to enforce gating by default)
...
async def lifespan(app: FastAPI):
    # Validate env at startup (so import never explodes)
...
async def get_trading_goals(user_id: str = Depends(get_current_user), conn = Depends(get_db)):
    rows = await conn.fetch("SELECT * FROM trading_goals WHERE user_id=$1 ORDER BY created_at DESC", user_id)
...
# -------------------
# Trades: list (GET) and create (POST)
# -------------------
@app.get("/api/trades")
async def list_trades(
    limit: int = 50,
    offset: int = 0,
    sort: str = "timestamp_desc",   # "timestamp_desc" | "timestamp_asc" | "profit_desc" | "profit_asc"
    ...
    params = [user_id]
    if tag:
        where.append(f"tag = ${len(params)+1}")
        params.append(tag)
    if platform:
    ...
    # Total
    total = await conn.fetchval(f"SELECT COUNT(*) FROM trades WHERE {where_sql}", *params)

    # Page
    params_page = params + [limit, offset]
    rows = await conn.fetch(
        f"""
        SELECT id, player, version, buy, sell, quantity, platform, profit, ea_tax, tag, timestamp
        FROM trades
        WHERE {where_sql}
        ORDER BY {order_sql}
...
async def bulk_edit_trades(request: Request, user_id: str = Depends(get_current_user), conn = Depends(get_db)):
    try:
        ...
        query = f"""
            ...
            WHERE user_id = ${len(params)-1} AND id = ANY(${len(params)})
        """
...
async def import_trades(file: UploadFile = File(...), user_id: str = Depends(get_current_user), conn = Depends(get_db)):
    try:
        ...
        if file.filename.endswith('.json'):
            ...
            trades_to_import = raw if isinstance(raw, list) else [raw]
        ...
        return {"message": f"Successfully imported {imported} trades", "imported_count": imported, "errors": errors[:10]}
    except Exception as e:
...
async def search_players(q: str = "", user_id: str = Depends(get_current_user), conn = Depends(get_db)):
    query = q.strip()
    if not query:
        return {"players": []}
    rows = await conn.fetch("""
