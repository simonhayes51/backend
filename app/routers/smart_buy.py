...
# ---------- Suggestions ----------
async def _candidate_cards(min_rating: int, max_rating: int, limit: int = 120) -> List[asyncpg.Record]:
    try:
        async with (await pool()).acquire() as c:
            return await c.fetch("""
              SELECT card_id, name, rating, position, league, nation
              FROM fut_players
...
async def _score(card: asyncpg.Record, platform: str, budget: int, horizon: str) -> Optional[Dict[str, Any]]:
    price = await _safe_price(int(card["card_id"]), platform)
    if not price or price <= 0 or price > budget:
        return None
    ...
    value_ratio = rating / max(2.0, math.log(price))
    bias = {"quick_flip": 1.2, "short": 1.0, "long_term": 0.8}.get(horizon, 1.0)

    score = ((value_ratio * 10) - 0.4 * mom24 - 0.2 * mom4) * bias
    target = round(price * (1.08 if mom4 <= 0 else 1.04))
    tax = int(target * 0.05)
    est = max(0, target - tax - int(price))
...
async def suggestions(
    ...
    cards = await _candidate_cards(min_rating, max_rating, 120)
