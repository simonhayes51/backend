...
# Uses your existing price getter
# Ensure your worker service has PYTHONPATH=. so this import works
...
async def _ensure_schema(conn: asyncpg.Connection) -> None:
    ...
    # NOTE: We are not changing the type of `price` here.
    # If you later migrate `price` to INTEGER, see the comment near _update_prices().
...
async def _fetch_one_price(card_id_text: str, platform: str, retries: int) -> Optional[int]:
    """Fetch a single price; returns int price or None."""
    try:
...
async def _update_prices(
    ...
    Current schema has fut_players.price as TEXT, so we cast price to TEXT in SQL.
    """
    now = _now_utc()
    rows: List[Tuple[str, str, datetime]] = []
    for cid, price in price_map.items():
        if isinstance(price, int):
            # keep tuple order matching SQL placeholders
            rows.append((cid, str(price), now))  # $1 card_id (TEXT), $2 price as TEXT, $3 timestamp

    if not rows:
        return len(price_map), 0

    await conn.executemany(
        """
        UPDATE fut_players
        SET price = $2::text, price_updated_at = $3
...
async def _refresh_once(pool: asyncpg.Pool) -> None:
    ...
    # Process in batches to control memory and log cadence
