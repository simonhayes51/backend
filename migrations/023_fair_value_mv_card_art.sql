-- Migration 023: expose card-art columns through fair_value_mv
-- target: player
-- requires-table: fut_players, sales_history, bin_history
--
-- Postgres has no ALTER MATERIALIZED VIEW for the underlying query, only
-- DROP + CREATE (same reason migration 013 had to rebuild 011/012's
-- definition rather than edit it) - this rebuilds 013's definition
-- unchanged except for passing through fut_players' new card_bg_image/
-- card_cutout_image/card_cutout_type/card_name columns (migration 022),
-- so /api/dashboard/stats' Movers section and /api/v2/market/fair-value/
-- batch get real card art with no extra join of their own.
